...
namespace cms_webservice.DAL
{
    ...
    {
        #region SQL
        public const string sqlGetPetitionList = "SELECT T_Petitions.*, T_Pic.imageUrl FROM T_Petitions LEFT OUTER JOIN T_Pic ON T_Petitions.id = T_Pic.petitionId";
        ...
        public const string sqlGetPetitionById = "SELECT T_Petitions.*, T_Pic.imageUrl FROM T_Petitions LEFT OUTER JOIN T_Pic ON T_Petitions.id = T_Pic.petitionId WHERE (T_Petitions.id = {0})";
